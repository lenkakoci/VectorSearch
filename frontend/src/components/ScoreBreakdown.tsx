import { formatScore } from '../lib/modes'
...
// Why a chunk sits where it sits: its place in each branch and the fusion sum.
    ...
    <div className="rounded-md bg-slate-50 p-3 text-xs">
      <table className="w-full">
        <tbody>
          ...
          {row('Fulltext (slova)', hit.fts_rank, hit.fts_score, 'ts_rank', 'fts')}
          {hit.rrf_score != null && (
            <tr className="border-t border-slate-100">
      ...
      <p className="mt-2 text-slate-500">
        {hit.lexical_match
          ? 'Text obsahuje hledaná slova (v některém tvaru).'
