import { formatScore, MODE_INFO, MODES } from '../lib/modes'
...
function Block({ response }: { response: SearchResponse }) {
  ...
  const facts: [string, unknown][] = [
    ...
    ['limit / over-fetch na větev', `${response.limit} / ${response.fetch}`],
    ['RRF k', debug.rrf_k],
    ...
    ['fulltextová větev', debug.fts_ms != null ? `${debug.fts_candidates} kandidátů · ${debug.fts_ms} ms` : 'neběžela'],
        ...
        <table className="w-full text-xs">
          <thead className="text-left text-slate-500">
            <tr>
              ...
              <th className="py-1 pr-2">ts_rank</th>
        ...
        {state.compare &&
          MODES.map((mode) => (
            <div key={mode}>
              ...
              <Block response={state.compare![mode]} />
            </div>
          ))}
      </div>
    </details>
