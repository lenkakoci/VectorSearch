...
export default function App() {
  ...
  const submit = (text: string) => {
    ...
    void run(trimmed, view, toRequestFilters(filters), limit)
  }

  // A change of mode, filter or limit re-runs the last query, so the demo can
  // flip between views without retyping. The query embedding is cached server
  // side, so this costs SQL only.
  ...
  useEffect(() => {
    ...
    if (lastQuery.current) void run(lastQuery.current, view, toRequestFilters(filters), limit)
  }, [view, filters, limit, run])
    ...
    <div className="mx-auto max-w-7xl space-y-4 p-4 md:p-6">
      <header className="flex flex-wrap items-baseline justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-slate-500">Demo vyhledávání v geologických posudcích: slova × význam × obojí</p>
        </div>
      ...
      {state.status === 'idle' && (
        <div className="rounded-lg border border-dashed border-slate-300 bg-white p-8 text-center text-slate-500">
          Zadejte dotaz nebo klikněte na některý z ukázkových. Režim „Porovnat“ ukáže stejný dotaz třemi způsoby vedle sebe.
        </div>
      )}
